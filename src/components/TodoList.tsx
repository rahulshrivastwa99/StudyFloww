...
import { Plus, Search, Filter, Calendar, CheckCircle2, Circle, Trash2, Edit3 } from 'lucide-react';

const TodoList: React.FC = () => {
  const { todoItems, addTodoItem, updateTodoItem, deleteTodoItem } = useStudyData();
        ...
        <div className="space-y-4">
          {filteredItems.length === 0 ? (
            ...
            filteredItems.map(item => (
              <div
                key={item.id}
                className={`p-6 rounded-lg border-l-4 shadow-lg transition-all duration-200 hover:shadow-xl ${getPriorityColor(item.priority)} ${
                  item.completed ? 'opacity-70' : ''
                }`}
              >
                <div className="flex items-start gap-4">
                  <button
                    onClick={() => updateTodoItem(item.id, { completed: !item.completed })}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-2">
                      ...
                      <div className={`w-3 h-3 rounded-full ${getPriorityDot(item.priority)}`}></div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-4 text-sm text-gray-500 dark:text-gray-400">
                      ...
                      <span className="capitalize px-2 py-1 rounded-full text-xs bg-opacity-20" style={{
